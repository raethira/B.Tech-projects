 ...
 *
 */
public class Student_c_teaches {
      public static void main(String args[], String user,String course) throws SQLException
    {
		...
		int i=0;
                ...
                String[] ColumnNames = {"TEACHER ID","TEACHER NAME"};
                Object[][] Contents = new Object[10][2];
                while(results.next())
                {if(results.getString(3).matches(user))
                {if(results.getString(2).matches(course))
                {               teacher[i]=results.getString(1);
                                 Contents[j][0]= teacher[i];
	     ...
	     j++;
                ...
                for(i=0;i<k;i++)
                {
                   ResultSet res = st.executeQuery("select tid,tname from tdetails");
                while(res.next())
               {
                
                String s=""+res.getString(1);
                String x=""+res.getString(2);
                if(teacher[i].matches(s)==true)
                {
                 Contents[j][1]=x;
                 System.out.println(teacher[i]+" "+Contents[j][1] );
                 //Contents[j][1]=x;
                //i++;
                j++;
                }
                ...
                //
                Scteacher obj = new Scteacher(Contents, ColumnNames,user);
                obj.setVisible(true);
